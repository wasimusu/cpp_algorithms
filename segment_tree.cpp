// Range queries for sum using Segment Trees
// It is better used for min, max queries
...
int query(int *tree, int N, int nodeL, int nodeR, int queryL, int queryR, int node) {
    ...
    int ll = query(tree, N, nodeL, mid, queryL, queryR, 2 * node + 1);
    int rr = query(tree, N, mid + 1, nodeR, queryL, queryR, 2 * node);
...
int main() {
    ...
    for (int i = 0; i < 1; i++) {
        cin >> l >> r;
        cout << l << ", " << r << "\t" << endl;
        auto res = query(tree, N, 0, k - 1, l, r, 1);
